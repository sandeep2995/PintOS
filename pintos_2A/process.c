...
//		printf("\n%s\n", *args);
  ...
  if (!success) 
    thread_exit ();
//printf"\nloaded successsfuly returnes and now assembly code\n\n");
//printf("\n%p\n", if_.esp);
hex_dump(0, if_.esp, 128, true);
//	printf("\nasffa: %d\n", **(char **)(if_.esp + 2*sizeof(int)));
...
load (const char **file_args, void (**eip) (void), void **esp) 
{
  ...
  file = filesys_open (file_args[0]);//printf"\n\n\n\nyo\n");
  if (file == NULL) 
    {
