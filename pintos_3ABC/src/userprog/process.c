  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      if (kpage == NULL)
      {
        ...
        /*****Implement the page eviction strategy here***********///3//
...
install_page (void *upage, void *kpage, bool writable)
{
  struct thread *t = thread_current ();
	...
	if(ret_val)
	{
		...
		set_spt(upage, 0, NULL, -1, -1, writable);
	}	
	/************************************************************************/
...
void set_spt(uint8_t *upage, int type, struct file *file, int32_t ofs, int swap_slot, bool writable)
{
	...
	for(e=list_begin(&cur->sup_pt); e!=list_end(&cur->sup_pt); e=list_next(e))
	{
		...
		{
			page->type = type;
			//to do: implement use of file, ofs and swap slot as per requirement
			break;
		}
	}
	if(e==list_end(&cur->sup_pt))
	{
			struct spt *cur_page = (struct spt *)malloc(sizeof(struct spt));
			cur_page->v_addr = upage;
			cur_page->type = type;
			...
			cur_page->ofs = ofs;
			cur_page->writable = writable;
			list_push_back(&cur->sup_pt, &cur_page->elem);
